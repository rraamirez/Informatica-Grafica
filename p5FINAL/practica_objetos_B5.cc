...
typedef enum{CUBO, PIRAMIDE, TRONCO_PIRAMIDE, OBJETO_PLY, ROTACION, ROTACION_PLY, CILINDRO, CONO, ESFERA, EXTRUSION,
             MONTANA, EXCAVADORA, /*pruebas*/MOTOR, ALA, CANION, PUERTA, CABINA, PUNTA, /*MODELO FINAL*/ ALAX, TEXTURA} _tipo_objeto;
...
void draw_objects()
{
    ...
    case MONTANA:
        ...
        break;
    case ALAX://???
        alaX.draw(modo, 0.15, 0.55, 0.25, 5);
        break;
    case TEXTURA:
        dibuja_cubo();
        break;
...
void normal_key(unsigned char Tecla1, int x, int y)
{
    ...
    case 'M':
        ...
        //MOVIMIENTO DESACTIVADO: VER PRACTICA 3.
...
void procesar_color(unsigned char color[3])
{
    ...
    for (i = 0; i < alaX.piezas; i++)
    {
        if (color[0] == alaX.color_select[i].r &&
            color[1] == alaX.color_select[i].g &&
            color[2] == alaX.color_select[i].r)
        {
            if (alaX.activo[i] == 0)
            {
                alaX.activo[i] = 1;
